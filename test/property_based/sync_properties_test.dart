...
import 'package:glimfo_todo/feature/todo/data/datasources/supabase_task_datasource.dart';
import 'package:glimfo_todo/feature/todo/data/repositories/task_repository_impl.dart';
...
void main() {
  group('Sync Properties - Offline-Online Consistency', () {
    ...
    PropertyTestRunner.runPropertyWithGenerator<TaskEntity>(
      ...
      iterations: PropertyTestConfig.getSettingsFor('sync_operations').iterations,
    ...
    PropertyTestRunner.runPropertyWithGenerator<List<TaskEntity>>(
      ...
      property: (tasks) {
        ...
        when(mockHiveDataSource.getTaskById(olderTask.id.toString()))
    ...
    PropertyTestRunner.runPropertyWithGenerator<TaskEntity>(
      description: 'Offline task updates sync to remote when connectivity restored',
      generator: () => TaskGenerators.generateValidTask(),
      property: (task) {
        // Setup: Start offline
        when(mockNetworkInfo.isConnected).thenAnswer((_) async => false);
    ...
    PropertyTestRunner.runPropertyWithGenerator<TaskEntity>(
      description: 'Offline task deletions sync to remote when connectivity restored',
      generator: () => TaskGenerators.generateValidTask(),
      property: (task) {
        // Setup: Start offline
        when(mockNetworkInfo.isConnected).thenAnswer((_) async => false);
    ...
    PropertyTestRunner.runPropertyWithGenerator<TaskEntity>(
      description: 'Syncing the same task multiple times produces consistent results',
      generator: () => TaskGenerators.generateValidTask(),
      property: (task) {
        // Setup: Online
        when(mockNetworkInfo.isConnected).thenAnswer((_) async => true);
