...
class BackgroundSyncService {
  ...
  final Duration _syncInterval = AppDurations.cacheMedium;
  final Duration _retryDelay = AppDurations.retryMax;
  int _retryCount = 0;
  ...
  void startPeriodicSync() {
    if (_syncTimer?.isActive == true) {
      developer.log('Background sync already running',
          name: 'BackgroundSyncService');
      return;
    }

    developer.log('Starting background sync service',
        name: 'BackgroundSyncService');
  ...
  void stopPeriodicSync() {
    developer.log('Stopping background sync service',
        name: 'BackgroundSyncService');
    _syncTimer?.cancel();
    _syncTimer = null;
    _updateSyncStatus(SyncStatus.idle);
