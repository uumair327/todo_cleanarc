  ...
  Future<void> build(BuildStep buildStep) async {
    ...
    try {
      ...
      if (!result.isValid) {
        ...
        // Log violations
        log.severe(
            'Color validation failed with ${result.violations.length} violations:');
        for (final violation in result.violations.take(5)) {
          // Show first 5
          log.severe(
              '  ${violation.filePath}:${violation.lineNumber} - ${violation.colorValue}');
        }
        ...
        throw BuildException(
            'Color validation failed. Found ${result.violations.length} hardcoded colors.');
      } else {
        log.info(
            'Color validation passed (${result.filesScanned} files scanned)');
